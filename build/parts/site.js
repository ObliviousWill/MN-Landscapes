...
/* ── Hero carousel ───────────────────────────────────────────────────
   Crossfades every 6 seconds. Stops on hover, on keyboard focus, when the
   tab is hidden, when the pause button is pressed, and never starts at all
   if the visitor has asked for reduced motion. */
(function(){
  ...
  var DELAY=6000, i=0, timer=null, stopped=false;
  ...
  function start(){
    if(timer||stopped||reduced.matches||document.hidden) return;
    timer=setInterval(tick,DELAY);
  }
  function halt(){ if(timer){ clearInterval(timer); timer=null; } }

  dots.forEach(function(d){
    d.addEventListener('click',function(){ show(+d.dataset.go); halt(); start(); });
  });

  pauseBtn.addEventListener('click',function(){
    stopped=!stopped;
    pauseBtn.dataset.paused=stopped?'true':'false';
    pauseBtn.setAttribute('aria-label',stopped?'Play the pictures':'Pause the pictures');
  ...
  if(reduced.matches){
    ...
    pauseBtn.setAttribute('aria-label','Play the pictures');
  }
  start();
})();
